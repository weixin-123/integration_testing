...
class DataFractionation:
    ...
    # 总已还债权微币
    def dev_all_to_be_completed_first(self):
        pay_datas = DevDebtData().query_dev_all_agent_detail(1)
        print(pay_datas)
        sum = 0
        for i in pay_datas:
            ...
            # print("期数：",periods)
            # print("每月还款额度：", repayment_quota)
            all_quota =periods * repayment_quota
            # print("待还款微币为：", all_quota)
            sum = sum + all_quota
        # 取绝对值
        # print(abs(sum))
    ...
    def dev_all_to_be_completed_second(self):
        pay_datas = DevDebtData().query_dev_all_agent_detail(3)
        sum = 0
        for i in pay_datas:
            periods = i[0]
            completed_cash = i[1]
            all_cash = periods * completed_cash
            sum = sum + all_cash
        return sum
    ...
    def dev_cash_completed_second_step(self):
    ...
    def dev_cash_completing_first_step(self):
        sql_data = DevDebtData().completing_debt_not_cash()
        total = len(sql_data)
        temp_list = []
        for i in range(total):
            # print('本金为:', sql_data[i][0])
