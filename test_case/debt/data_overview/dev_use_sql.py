...
@File    : dev_use_sql.py
"""
from test_case.debt.data_overview.dev_sql import DevSql
from common.log_color import LogingColor

logging = LogingColor()
dev_db = DevSql()
begin_time = '2020-12-22 00:00:00'
end_time = '2020-12-22 18:00:00'
...
class DevDebtData:
    ...
    def completing_coins(self, order_id):
        sql = "SELECT repayment_amount_sum,number_periods FROM life_debt_order_list where order_id = '%s' and state = 2  order_manage by number_periods desc" % order_id
        sql_data = dev_db.query_sql('debt', sql)
        if sql_data == "" or sql_data is None:
            logging.error("查询数据库失败，看下sql是不是写得有问题，还是数据库连错了~~~~~~~~")
            return "False"
    ...
    def query_dev_agent_detail(self, order_id, state):
        sql = "SELECT number_periods, money, repayment_amount_sum, state FROM life_debt_order_list " \
              "where order_id  = '%s' and state = '%s' order_manage by number_periods desc" % (order_id, state)
        sql_data = dev_db.query_sql('debt', sql)
        if sql_data == "" or sql_data is None:
            logging.error("查询数据库失败，看下sql是不是写得有问题，还是数据库连错了~~~~~~~~")
            return "False"
    ...
    def query_dev_all_agent_detail(self, state):
        sql = "SELECT repayment_period, repayment_quota FROM life_debt_order_apply where repayment_status = '%s' and debtor_mobile = 15900606723 and created_at between '%s' and '%s'" % (state, begin_time, end_time)
        sql_data = dev_db.query_sql('debt', sql)
        if sql_data == "" or sql_data is None:
            logging.error("查询数据库失败，看下sql是不是写得有问题，还是数据库连错了~~~~~~~~")
            return "False"
    ...
    def completed_debt_cash(self):
        sql = "SELECT amount,interest_amount FROM life_debt_order_apply where status in (60,130) and debtor_mobile = 15900606723 and created_at between '%s' and '%s'" % (begin_time, end_time)
        sql_data = dev_db.query_sql('debt', sql)
        if sql_data == "" or sql_data is None:
            logging.error("查询数据库失败，看下sql是不是写得有问题，还是数据库连错了~~~~~~~~")
            return "False"
    ...
    # 待完成解债金额  60 130
    def completing_debt_not_cash(self):
        sql = "SELECT amount,interest_amount FROM life_debt_order_apply where status not in (60,130) and debtor_mobile = 15900606723 and created_at between '%s' and '%s'" % (begin_time, end_time)
        sql_data = dev_db.query_sql('debt', sql)
        if sql_data == "" or sql_data is None:
            logging.error("查询数据库失败，看下sql是不是写得有问题，还是数据库连错了~~~~~~~~")
            return "False"
    ...
    def completed_debt_cash_total(self):
        sql = "SELECT count(id) FROM life_debt_order_apply where status in ('130') and debtor_mobile = 15900606723 and created_at between '%s' and '%s'" % (begin_time, end_time)
        sql_data = dev_db.query_sql('debt', sql)
        if sql_data == "" or sql_data is None:
            logging.error("查询数据库失败，看下sql是不是写得有问题，还是数据库连错了~~~~~~~~")
            return "False"
    ...
    def completed_debtor(self):
        sql = "SELECT amount,interest_amount,reduction_amount FROM life_debt_order_apply where debtor_mobile = 15900606723 and created_at between '%s' and '%s'" % (begin_time, end_time)
        sql_data = dev_db.query_sql('debt', sql)
        if sql_data == "" or sql_data is None:
            logging.error("查询数据库失败，看下sql是不是写得有问题，还是数据库连错了~~~~~~~~")
            return "False"
